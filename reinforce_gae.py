...
class Policy(nn.Module):
    ...
    def calculate_loss(self, gamma=0.999, lambda_=0.99):
        ...
        gae_estimator = GAE(gamma=gamma, lambda_=lambda_, num_steps=len(rewards))
        est_adv_list = gae_estimator(rewards, state_value_list) # list of estimated advantage in each time step
...
class GAE:
    ...
    def __call__(self, rewards, values):
        ...
        for step in range(self.num_steps-1, -1, -1):
            reward = rewards[step]
            value = values[step]
            td_err = reward + self.gamma * next_value - value # td_error = r_t+1 + gamma * v(s_t+1) - v(s_t)
            adv = td_err + self.gamma * self.lambda_ * adv # A_t = td_err + gamma * lambda * A_t+1
            adv_list.insert(0, adv)
            next_value = value
        est_adv_list = torch.tensor(adv_list)
    ...
    for episode in range(args.episodes):
        ...
        if ewma_reward > env.spec.reward_threshold or episode == args.episodes-1:
            if not os.path.isdir("./models"):
                os.mkdir("./models")
            torch.save(model.state_dict(), f"./models/{args.env}_gae.pth")
            break
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser("REINFORCE algorithm with GAE")
    parser.add_argument("--env", type=str, default="LunarLander-v2", help="Name of the environment")
    parser.add_argument("--seed", type=int, default=10, help="Random seed")
    parser.add_argument("--lr", type=float, default=0.01, help="Learning rate")
    parser.add_argument("--step_size", type=int, default=300, help="Step size for lr scheduler")
    parser.add_argument("--episodes", type=int, default=7000, help="Number of episodes for training")
    parser.add_argument("--gamma", type=float, default=0.999, help="Discount factor")
    parser.add_argument("--gae_lambda", type=float, default=0.99, help="Lambda for GAE")
